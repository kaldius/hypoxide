...
#[repr(u8)] // ensures each enum variant is stored as a u8
pub enum Color {
    Black = 0,
    Blue = 1,
    Green = 2,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)] // ensures this struct has the exact same data layout as u8
struct ColorCode(u8);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(C)] // field ordering is undefined in Rust, this attribute ensures an ordered, C-like layout
struct ScreenChar {
    ascii_character: u8,
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                ...
                // not part of printable ASCII range, print ■ character
...
lazy_static! {
    pub static ref WRITER: Mutex<Writer> = Mutex::new(Writer {
        ...
        // the cast only succeeds because of
        // #[repr(transparent)] and #[repr(C)] attributes on the types used in Buffer
...
mod test {
    ...
    fn println_output() {
        ...
        interrupts::without_interrupts(|| {
            ...
            // print a `\n` before the string to clear out potential `.`s from timer interrupts
            writeln!(writer, "\n{}", s).expect("writeln failed");
            for (i, c) in s.chars().enumerate() {
                let screen_shar = writer.buffer.chars[BUFFER_HEIGHT - 2][i].read();
                assert_eq!(screen_shar.ascii_character as char, c)
            }
        })
    }
}
